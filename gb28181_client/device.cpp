...
#include "../3rd/pugixml/include/pugixml.hpp"
...
void Device::start() {
    ...
    char temp_ip[64] = { 0 };
    // local ip & port
    eXosip_guess_localip(sip_context, AF_INET, temp_ip, sizeof(temp_ip));
    local_ip = string(temp_ip);
...
void Device::process_request() {
    while (is_running) {
        ...
        default: {
            spdlog::info("unhandled sip evt type: {}", evt->textinfo[evt->type]);
            break;
        }
        }
    }
